...
LQNetKit 是一个 Swift 编写的通用网络请求工具库，支持 GET/POST/PUT/DELETE、参数自动编码、响应链式处理、错误映射、缓存、进度回调、网络状态监听、断网自动重试、并发队列、优先级、全局拦截器、日志、文件上传下载、流式响应、async/await、Mock/Stub 等。
  DESC
  s.homepage     = "https://github.com/xiangduojia/LQNetKit"
  s.license      = { :type => "MIT", :file => "LICENSE" }
  s.author       = { "xiangduojia" => "xiangduojia@example.com" }
  s.source       = { :git => "https://github.com/xiangduojia/LQNetKit.git", :tag => s.version }
  s.platform     = :ios, "12.0"
  s.swift_version = "5.0"
  s.source_files = "LQNetKit/LQNetKit.swift"
  s.requires_arc = true
end
